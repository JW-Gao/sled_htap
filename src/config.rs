...
pub struct Inner {
    ...
    #[doc(hidden)]
    pub segment_size: usize, // 这个不知道是干嘛的
    #[doc(hidden)]
    pub path: PathBuf,
    #[doc(hidden)]
    pub create_new: bool,
    ...
    #[doc(hidden)]
    pub idgen_persist_interval: u64, // 不知道这是干啥的，为啥要有个‘区间’呢
    #[doc(hidden)]
...
impl Default for Inner {
    fn default() -> Self {
        Self {
            ...
            mode: Mode::LowSpace, // 默认竟然不是 high Throughput吗
...
impl Config {
    ...
    pub fn open(&self) -> Result<Db> {
        ...
        let mut config = self.clone(); //为什么要clone一下
