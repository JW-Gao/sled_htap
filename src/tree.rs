...
impl Tree {
    ...
    pub(crate) fn insert_inner(
        ...
        // 为什么这里要去找目标key，不能直接插入吗
        // 我知道了，总体上还是要维持一个b树的结构
        let View { node_view, pid, .. } =
            self.view_for_key(key.as_ref(), guard)?; // 获取目标节点view，此处应该是有traverse的
        ...
        let (encoded_key, last_value) = node_view.node_kv_pair(key.as_ref()); // 要不就是在这里找
    ...
    pub(crate) fn view_for_pid<'g>(
        ...
        loop {
            // 由page cache模块负责相应 pid 的Node吗？，怎么保证lock-gree的呢
    ...
    // 当使用lock-free的方法时，一个节点可能处于以下状态
    // 1. 正常状态 2. 分裂中 3. 合并中
    // 当处于分裂中时，可以处于分裂步骤中的任意一步完成时
    // 同理，当处于合并中时，也可以处于合并步骤的任意一步完成时
    // 我们要确保无论处于哪个状态，该状态都能被有效地记录在二级存储中
    // 并且在故障恢复后和故障前的状态保持一致
    pub(crate) fn merge_node<'g>(
        ...
        let mut merge_index = child_index - 1; // 数据被合并到的页id
