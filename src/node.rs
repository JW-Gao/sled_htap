...
fn shift_distance(
    ...
    // 只在buf 的后 `shift` 位上进行 buf + distance，超过shifit位后终止
    // 如：'a b c ' + 'b c' 在后两位（shift = 2）
    // 在
    while shift > 0 {
...
impl Node {
    ...
    pub(crate) fn apply(&self, link: &Link) -> Node {
        ...
        match *link {
            ...
            ParentMergeIntention(pid) => { // 先更新 parent 中的索引导航部分吗
                assert!(
    ...
    fn insert(&self, key: &IVec, value: &IVec) -> Node {
        // 好牛逼啊 !!!
        // 在rust中，对Arc类型的值调用clone只会触发浅拷贝（数据不会真的复制一次）
        // 只影响当前线程的可见性
    ...
    // 要保证node overlay是已经合并过的，也就是overlay.is_empty() == true
    ...
    pub(crate) fn should_split(&self) -> bool {
        ...
        let size_checks = if cfg!(any(test, feature = "lock_free_delays")) {
            self.iter().take(6).count() > 5 // 这单个Node的容纳量也太少了吧，难道是 用来存储 <= 5 个delta 吗？？？？
        } else {
...
impl Inner {
    ...
    fn find(&self, key: &[u8]) -> Result<usize, usize> {
        ...
        if size == 0 || self.index_key(0).unwrap_slice() > key { // lo > key的情况是错误的， lo <= key < hi
            return Err(0);
        }
        let mut left = 0;
        let mut right = size;
    ...
    fn index_key(&self, idx: usize) -> KeyRef<'_> {
        ...
        let offset_sz = self.children as usize * self.offset_bytes as usize;
        /*
         *整体inner布局如下(index node， 而非数据node)
         * | header | lo | hi |     data      |
         * data部分又分为
         * | key offset | （key, key_offset) pair |
         * 
         */
