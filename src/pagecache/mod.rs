...
// 很牛逼啊，NodeView -> PageView, PageVeiw 获取当前pid对应的Node(Update.as_node())，在in_mem Node中找寻目标key
...
pub(in crate::pagecache) enum Update { // 有关Update的metadata在哪里呢，更新是针对什么的呢
    Link(Link), // 页面数据操作
    Node(Node), // 页面数据操作， Node 可以 apply link
    Free,   // 剩下的是状态吗
    Counter(u64),
    Meta(Meta),
}
...
impl PageCache {
    ...
    pub(crate) fn link<'g>(
        ...
        // 为什么这一步没有在loop里面
        // 线程A和线程B，同时更新这一个Node，怎么办呢
        // 不会，因为CAS是原子操作，有最终一致性，若是其它线程更换成功，本次更新直接失败
        ...
        loop {
            ...
            let mut new_cache_infos =
                Vec::with_capacity(old.cache_infos.len() + 1); // 这里一定要clone原理的吗，一定！！，因为要无锁
...
impl PageCacheInner {
    ...
    fn allocate_inner<'g>(
        ...
        let (pid, page_view) = if let Some(pid) = free_opt {
            ...
            // much more easily.
            allocation_serializer = self.next_pid_to_allocate.lock(); // 在c++中，此处应该用scoped_lock，防止忘记unlock
    ...
    /*pull 的作用是：根据页面ID、日志序列号和磁盘位置，
    从底层日志文件或堆文件中拉取（读取）该页面的内容，
    并反序列化为内存结构体 Update，用于内存中的重建或缓存 */
