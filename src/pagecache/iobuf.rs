...
impl IoBufs {
    pub fn start(config: RunningConfig, snapshot: &Snapshot) -> Result<IoBufs> {
        ...
        // 我懂了，next_lid 处在还未用完的segment中，用这个segment剩下的部分与iobuf映射
        // 这就导致某些io_buf可能达不到segment_size的大小
    ...
    pub(crate) fn write_to_log(&self, iobuf: Arc<IoBuf>) -> Result<()> {
        ...
        let bytes_to_write = header::offset(header); // 那这里就可以说明header::offset指向空白区域
        ...
        // 为了在剩余空间还能容下一个msg_header的情况下，
        // 保证恢复的安全性，明确告诉恢复过程这里是正常结束的，不需要再继续读取msg_header了
