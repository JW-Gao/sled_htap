...
impl PageTable {
    ...
    pub(crate) fn insert<'g>(
        &self,
        pid: PageId,
        item: Page, // 为什么这里是page，不应该是Node吗，不懂欸
        guard: &'g Guard,
    ) -> PageView<'g> {
        debug_delay();
    ...
    pub(crate) fn get<'g>(
        ...
        debug_delay();
        let res = tip.load(Acquire, guard); // Shared 只读
    ...
    fn traverse<'g>(&self, k: PageId, guard: &'g Guard) -> &'g Atomic<Page> {
        ...
        // 为什么l2_ptr.is_null()的时候，要手动分配一个
        // A：延迟初始化，首次访问该pid，证明需要有该pid的page了
        if l2_ptr.is_null() {
...
impl Drop for PageTable {
    fn drop(&mut self) {
        let guard = pin(); // pins the current thread是什么意思
