...
impl Heap {
    pub fn start<P: AsRef<Path>>(p: P) -> Result<Heap> {
        ...
        for slab_id in 0..32 {
            let slab = Slab::start(&p, slab_id)?; // 也没有加 1 啊
            slabs_vec.push(slab);
        }
    ...
    pub fn gc_unknown_items(&self, snapshot: &crate::pagecache::Snapshot) {
        ...
        for slab in &self.slabs {
            ...
            bitmaps.push(vec![0_u64; 1 + (tip / 64)]); // 有点像lsm-tree的level consolidation啊
        }
...
impl Slab {
    pub fn start<P: AsRef<Path>>(directory: P, slab_id: u8) -> Result<Slab> {
        ...
        let free = Arc::new(Stack::default()); // 应该拿stack当元数据吗
        ...
        let len = file.metadata()?.len(); // 新创建的应该会返回 0
        let max_idx = len / bs;
        log::trace!(
    ...
    fn read(
        ...
        let bs = slab_id_to_size(self.slab_id); // 难道说只分两块吗
