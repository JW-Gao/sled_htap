...
impl Log {
    ...
    fn reserve_inner<T: Serialize + Debug>(
        ...
        // serialized size = key.size() + value.size() + 3，且只是最新的增量的长度
        let serialized_len = item.serialized_size();
        // println!("serialized len is {}", serialized_len);
        ...
        loop {
            ...
            let reservation_lsn =
                iobuf.lsn + Lsn::try_from(buf_offset).unwrap(); // 也是类似innodb的逻辑lsn增量方式
            ...
            let message_header = MessageHeader {
                crc32: 0,
                kind,
                segment_number: SegmentNumber( // 原来实在这里跟Segment联系上的，但是lsn是严格递增且无上限的，难道说segment_number也是无上限的吗
